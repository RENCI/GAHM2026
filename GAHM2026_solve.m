...
%
%  Input:
%     GAHM_in        - GAHM data structure from GAHM2026_consistency
...
function GAHM_out = GAHM2026_solve(GAHM_in,GAHM_constants,fid)
...
for q=1:4      % loop through the 4 quadrants
    ...
    for i=1:3  % loop through the isotachs
        ...
        if flag(q,i)==1 || flag(q,i)==5
            ra=GAHM_in.turnangle(q,i)*LatNS/abs(LatNS);
            ...
            elseif Gcase == 2
                ...
                if ~isreal(SVorQuad_10_10)
                    fprintf (' %s %s %s %i %s %i %s\n', ...
                      'ERROR: Function GAHM2026_solve: @ time =',GAHM_datetime, ...
                      'Imaginary solution for Vortex isotach speed, quadrant=',q, ...
                      ' isotach=',i,'RUN TERMINATED');
                    fprintf (fid,' %s %s %s %i %s %i %s\n', ...
                      'ERROR: Function GAHM2026_solve: @ time =',GAHM_datetime, ...
                      'Imaginary solution for Vortex isotach speed, quadrant=',q, ...
                      ' isotach=',i,'RUN TERMINATED');
                    GAHM_out.flag_exit=9;
                    return
                end
                if SVorQuad_10_10 < 0
                    fprintf (' %s %s %s %i %s %i %s\n', ...
                      'ERROR: Function GAHM2026_solve: @ time =',GAHM_datetime, ...
                      'Solution for Vortex isotach speed < 0, quadrant=',q, ...
                      ' isotach=',i,'RUN TERMINATED');
                    fprintf (fid,' %s %s %s %i %s %i %s\n', ...
                      'ERROR: Function GAHM2026_solve: @ time =',GAHM_datetime, ...
                      'Solution for Vortex isotach speed < 0, quadrant=',q, ...
            ...
            if SVorQuad_10_10 > SVorMax_10_10
                fprintf (' %s %s %s %s %i %s %i %s\n', ...
                      'ERROR: Function GAHM2026_solve: @ time =',GAHM_datetime, ...
                      'Solution for Vortex isotach speed > Maximum vortex ', ...
                      'speed 0 quadrant=',q,' isotach=',i,'RUN TERMINATED');
...
function [Rmax, Ro, Bg, phi, A_qi, Rmic, Bgicmax, flag_qi] = ...
    ...
    while abs(Rmax_tol) > 1
        Rmic=Rmic+1;
        if Rmic>200
            fprintf (' %s %s %s %i %s %i %s %.2f\n', ...
               'ERROR: Function GAHM2026_solve @ time ',GAHM_datetime, ...
               'Rmax failed to converge in 200 iterations, quadrant=', ...
                q,'isotach=',i,'final tol=',Rmax_tol);
            fprintf (fid,' %s %s %s %i %s %i %s %.2f\n', ...
               'ERROR: Function GAHM2026_solve @ time ',GAHM_datetime, ...
               'Rmax failed to converge in 200 iterations, quadrant=', ...
                q,'isotach=',i,'final tol=',Rmax_tol);
            flag_qi=9;
...
function [Rmax, Ro, Bg, phi, A_qi, flag_qi] = ...
    ...
    if ~isreal(x)
        fprintf ('%s %s %s %i %s %i %s %s\n', ...
               'ERROR: Function GAHM2026_solve @ time ',GAHM_datetime, ...
               'fsolve returned imaginary values for Bg, c, quadrant=', ...
                q,'isotach=',i,'Try adjusting GAHM_constants.Bg0M', ...
                'or GAHM_constants.c0.');
        fprintf (fid,'%s %s %s %i %s %i %s %s\n', ...
               'ERROR: Function GAHM2026_solve @ time ',GAHM_datetime, ...
               'fsolve returned imaginary values for Bg, c, quadrant=', ...
                q,'isotach=',i,'Try adjusting GAHM_constants.Bg0M', ...
                'or GAHM_constants.c0.');
    ...
    elseif fsolve_exitflag <=0
        fprintf (' %s %s %s %i %s %i %s %i %s\n', ...
               'ERROR: Function GAHM2026_solve @ time ',GAHM_datetime, ...
               'fsolve did not converge solving Bg, c, quadrant=', ...
                q,'isotach=',i,'exitflag =',fsolve_exitflag, ...
                'Try adjusting GAHM_constants.Bg0M or GAHM_constants.c0.');
        fprintf (fid,' %s %s %s %i %s %i %s %i %s\n', ...
               'ERROR: Function GAHM2026_solve @ time ',GAHM_datetime, ...
               'fsolve did not converge solving Bg, c, quadrant=', ...
                q,'isotach=',i,'exitflag =',fsolve_exitflag, ...
                'Try adjusting GAHM_constants.Bg0M or GAHM_constants.c0.');
...
function [Bg, Bgic] = compute_Bg_iterative(time,B,Ro,Bg_in,i,q,fid)
  ...
  while abs(Bg_tol) > 0.001
      ...
      elseif Bgic==100
          fprintf (' %s %s %s %s %i %s %i %s %.4f %s %.2f %s %.4f %s %.4f\n', ...
             'ERROR Function compute_Bg: @ time ',time,'Bg failed to', ...
            'converge in 100 iterations, quadrant=',q,'isotach=',i,'B=',B, ...
            'Ro=',Ro,'Bg=',Bg,'final tol=',Bg_tol);
          fprintf (fid,' %s %s %s %s %i %s %i %s %.4f %s %.2f %s %.4f %s %.4f\n', ...
             'ERROR Function compute_Bg: @ time ',time,'Bg failed to', ...
            'converge in 100 iterations, quadrant=',q,'isotach=',i,'B=',B, ...
            'Ro=',Ro,'Bg=',Bg,'final tol=',Bg_tol);
          break
...
if ~isreal(y)
    fprintf (' %s %s %s %s %i %s %i %s %f.4 %s %f.2 %s %f.2 %s\n', ...
            'ERROR Function compute_Bg: @ time ',time,'fsolve returned', ...
            'imaginary value for Bg, quadrant=',q,'isotach=',i,'B=',B,'Ro=', ...
             Ro,'Bg=',y,'Try adjusting GAHM_constants.Bg0M');
    fprintf (fid,' %s %s %s %s %i %s %i %s %f.4 %s %f.2 %s %f.2 %s\n', ...
            'ERROR Function compute_Bg: @ time ',time,'fsolve returned', ...
            'imaginary value for Bg, quadrant=',q,'isotach=',i,'B=',B,'Ro=', ...
             Ro,'Bg=',y,'Try adjusting GAHM_constants.Bg0M');
end
...
if fsolve_exitflag <=0
    fprintf (' %s %s %s %s %i %s %i %s %f.4 %s %f.2 %s %f.2 %s %i %s\n', ...
             'ERROR Function compute_Bg: @ time ',time,'fsolve did not', ...
            'converge for Bg, quadrant=',q,'isotach=',i,'B=',B,'Ro=',Ro,'Bg=',y, ...
            'exitflag=',fsolve_exitflag,'Try adjusting GAHM_constants.Bg0M');
    fprintf (fid,' %s %s %s %s %i %s %i %s %f.4 %s %f.2 %s %f.2 %s %i %s\n', ...
             'ERROR Function compute_Bg: @ time ',time,'fsolve did not', ...
            'converge for Bg, quadrant=',q,'isotach=',i,'B=',B,'Ro=',Ro,'Bg=',y, ...
            'exitflag=',fsolve_exitflag,'Try adjusting GAHM_constants.Bg0M');
end
