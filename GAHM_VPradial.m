...
%     v3 eliminated extraneous output from GAHM_VP.m 7/6/2025
%
% Inputs:
...
function GAHM_VPrad = GAHM_VPradial(r,theta,GAHM_constants,GAHM)
        ...
        if ~isnan(GAHM.RQuad(q,3)) && ~isnan(GAHM.RQuad(q,2)) && ~isnan(GAHM.RQuad(q,1))
            if r(ir) <= GAHM.RQuad(q,3)
                [Press_RP(iRP),VVor_10_10_RP(iRP,:)]=...
                                 GAHM_VP(r(ir),q,3,GAHM_constants,GAHM);
            elseif r(ir) <= GAHM.RQuad(q,2)
                [Press3,VVor3]=GAHM_VP(r(ir),q,3,GAHM_constants,GAHM);
                [Press2,VVor2]=GAHM_VP(r(ir),q,2,GAHM_constants,GAHM);
            ...
            elseif r(ir) <= GAHM.RQuad(q,1)                
                [Press2,VVor2]=GAHM_VP(r(ir),q,2,GAHM_constants,GAHM);
                [Press1,VVor1]=GAHM_VP(r(ir),q,1,GAHM_constants,GAHM);
                ...
                [Press_RP(iRP),VVor_10_10_RP(iRP,:)]=GAHM_VP(r(ir),q,1,GAHM_constants,GAHM);
            end 

% Case 2 two isotachs (34, 50 kts) available          
        elseif ~isnan(GAHM.RQuad(q,2)) && ~isnan(GAHM.RQuad(q,1))
            if r(ir) <= GAHM.RQuad(q,2)
                [Press_RP(iRP),VVor_10_10_RP(iRP,:)]=GAHM_VP(r(ir),q,2,GAHM_constants,GAHM);
            elseif r(ir) <= GAHM.RQuad(q,1)
                [Press2,VVor2]=GAHM_VP(r(ir),q,2,GAHM_constants,GAHM);
                [Press1,VVor1]=GAHM_VP(r(ir),q,1,GAHM_constants,GAHM);
                R12_2=(GAHM.RQuad(q,1)-r(ir))/(GAHM.RQuad(q,1)-GAHM.RQuad(q,2));
...
% Case 4 no isotachs are available, use default values
            ...
            [Press_RP(iRP),VVor_10_10_RP(iRP,:)]=GAHM_VP(r(ir),q,4,GAHM_constants,GAHM);
        end 
