...
%     rank, source                           - group attributes
    ...
    for v = 1:length(info.Variables)
        ...
        % ncread() always applies scale_factor/add_offset and masks
        % _FillValue as NaN, so whenever either attribute is present the
        % data returned above is already unpacked. Those attributes (and
        % the packed-sentinel missing_value alias, if present) must not be
        % carried through to the output, or downstream CF-aware readers
        % would apply the unpacking formula a second time.
        unpacked = ~strcmp(orig_name, 'time') && ...
                   (any(strcmp({var.Attributes.Name}, 'scale_factor')) || ...
                    any(strcmp({var.Attributes.Name}, 'add_offset')));
        ...
        elseif any(strcmp(out_name, {'PSFC', 'U10', 'V10'}))
            ...
            if strcmp(out_name, 'PSFC') && strcmp(pressure_units, 'mb')
                % Only convert Pa -> mb if the source is actually in
                % Pascals; otherwise the incoming data is already
                % mb-equivalent (e.g. 'mb' or 'hPa') and dividing by 100
                % would corrupt it.
                units_idx1 = find(strcmp({var.Attributes.Name}, 'units'), 1);
                if isempty(units_idx1)
                    src_units = '';
                else
                    src_units = strtrim(var.Attributes(units_idx1).Value);
                end
                if strcmpi(src_units, 'Pa')
                    data = data / 100.0;
                    fprintf('  Converted PSFC from Pa to mb\n');
                else
                    fprintf('  PSFC source units ''%s'' are not Pa; skipping Pa->mb conversion\n', src_units);
                end
            end
        elseif unpacked
            nc_type = 'NC_DOUBLE';
        ...
        % Collect attributes (skip _FillValue — handled via defVarFill; skip
        % scale_factor/add_offset/missing_value when data is already
        % unpacked, since those attributes describe the packed
        % representation and no longer apply to the decoded values)
        attr_names  = {};
        attr_values = {};
        for a = 1:length(var.Attributes)
            ...
            if unpacked && any(strcmp(att.Name, {'scale_factor', 'add_offset', 'missing_value'}))
                continue;
            end
